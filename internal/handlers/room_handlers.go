...
func (r *RoomHandler) PostRoom(c *gin.Context) {
	...
	log.Printf("Successfully created room with ID: %v", res.ID)
...
func (r *RoomHandler) GetRoomInfo(c *gin.Context) {
	...
	if err != nil {
		log.Printf("Error getting room info by ID: %v", err)
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	log.Printf("Successfully got room info for ID: %v", id)
...
func (r *RoomHandler) PatchRoomInfo(c *gin.Context) {
	...
	if err != nil {
		log.Printf("Error editing room info: %v", err)
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	log.Printf("Successfully edited room info for ID: %v", roomIDInt)
	c.JSON(http.StatusOK, res)
}
...
func (r *RoomHandler) DeleteRoom(c *gin.Context) {
	...
	if err != nil {
		log.Printf("Error removing room by ID: %v", err)
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	log.Printf("Successfully deleted room with ID: %v", id)
...
func (r *RoomHandler) AddMemberToRoomHandler(c *gin.Context) {
	...
	if err != nil {
		log.Printf("Error adding member to room: %v", err)
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	log.Printf("Successfully added member with ID: %v to room with ID: %v", userID, roomID)
...
func (r *RoomHandler) RoomExistsMiddlewareByParam(paramKey string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !exists {
			log.Printf("Room with ID: %v does not exist", roomID)
...
func (r *RoomHandler) RoomExistsMiddlewareByJSON(jsonKey string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !ok {
			log.Printf("Invalid room ID in JSON input")
			c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": "invalid room ID"})
			return
		}
		...
		if !exists {
			log.Printf("Room with ID: %v does not exist", int(roomID))
			c.AbortWithStatus(http.StatusNotFound)
			return
		}
		c.Next()
...
func (r *RoomHandler) RoomPermissionsMiddleware(c *gin.Context) {
	...
	if err != nil {
		log.Printf("Error checking if user is owner of room: %v", err)
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	if !isOwner {
		log.Printf("User with ID: %v is not owner of room with ID: %v", userID, roomID)
...
func (r *RoomHandler) RoomAccessMiddlewareByParam(paramKey string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Printf("Error checking if user has access to room: %v", err)
			c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
		if !hasAccess {
