...
const (
	...
	SelectMembersByRoomIDQuery = `SELECT room_id, user_id FROM members WHERE room_id = $1`
...
const (
	...
	SoftDeleteMessagesByRoomIDQuery    = `UPDATE messages SET is_active = false, deleted_at = CURRENT_TIMESTAMP WHERE room_id = $1`
	SelectMessagePaginateQuery         = `SELECT id, sender_id, room_id, content, status, created_at, updated_at, deleted_at FROM messages WHERE is_active = true AND room_id = $1 LIMIT $2 OFFSET $3`
	SelectMessagesPaginateReverseQuery = `SELECT id, sender_id, room_id, content, status, created_at, updated_at, deleted_at FROM messages WHERE is_active = true AND room_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`
)
